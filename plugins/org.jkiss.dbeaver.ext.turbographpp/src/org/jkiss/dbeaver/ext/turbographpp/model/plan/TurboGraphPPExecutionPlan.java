...
import org.jkiss.dbeaver.ext.turbographpp.jdbc.proxy.driver.CUBRIDStatementProxy;
...
public class TurboGraphPPExecutionPlan extends AbstractExecutionPlan {
	...
	public TurboGraphPPExecutionPlan(JDBCSession session, String query) throws DBCException {
	    ...
	    this.query = query;
		//super((TurboGraphPPDataSource) session.getDataSource(), query);
		try {			
			
			CUBRIDStatementProxy proxy = new CUBRIDStatementProxy(session.getOriginal().createStatement());
			
			proxy.setQueryInfo(false);
