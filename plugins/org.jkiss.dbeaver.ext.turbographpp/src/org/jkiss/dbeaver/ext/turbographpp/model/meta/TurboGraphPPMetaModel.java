...
public class TurboGraphPPMetaModel {
    ...
    public TurboGraphPPDataSource createDataSourceImpl(
            DBRProgressMonitor monitor, DBPDataSourceContainer container) throws DBException {
    ...
    public JDBCBasicDataTypeCache<GenericStructContainer, ? extends JDBCDataType>
            createDataTypeCache(@NotNull GenericStructContainer container) {
    ...
    /**
     * Returns SQL clause for table column auto-increment. Null if auto-increment is not supported.
     */
    ...
    public List<GenericSchema> loadSchemas(
            JDBCSession session, GenericDataSource dataSource, GenericCatalog catalog)
            throws DBException {
        ...
        try {
            ...
            final DBSObjectFilter schemaFilters =
                    dataSource.getContainer().getObjectFilter(GenericSchema.class, catalog, false);
            ...
            if (catalog != null) {
                try {
                    ...
                    if (isSchemasOptional()) {
                        ...
                        log.debug(
                                "Error reading schemas in catalog '"
                                        + catalog.getName()
                                        + "' - "
                ...
                while (dbResult.next()) {
                    ...
                    String schemaName =
                            GenericUtils.safeGetString(
                                    schemaObject, dbResult, JDBCConstants.TABLE_SCHEM);
                    if (CommonUtils.isEmpty(schemaName)) {
                        // some drivers uses TABLE_OWNER column instead of TABLE_SCHEM
                        schemaName =
                                GenericUtils.safeGetString(
                                        schemaObject, dbResult, JDBCConstants.TABLE_OWNER);
                    }
                    ...
                    String catalogName =
                            GenericUtils.safeGetString(
                                    schemaObject, dbResult, JDBCConstants.TABLE_CATALOG);

                    if (!CommonUtils.isEmpty(catalogName)) {
                        if (catalog == null) {
                            if (!dataSource.isOmitCatalog()) {
                                // Invalid schema's catalog or schema without catalog (then do not
                                // use schemas as structure)
                                log.debug(
                                        "Catalog name ("
                                                + catalogName
                                                + ") found for schema '"
                                                + schemaName
                                                + "' while schema doesn't have parent catalog");
                            }
                        } else if (!catalog.getName().equals(catalogName)) {
                            if (!catalogSchemas) {
                                // Just skip it - we have list of all existing schemas and this one
                                // belongs to another catalog
                                continue;
                            }
                            log.debug(
                                    "Catalog name '"
                                            + catalogName
                                            + "' differs from schema's catalog '"
                                            + catalog.getName()
                                            + "'");
                        }
                    }
            ...
            if (dataSource.isOmitSingleSchema()
                    && catalog == null
                    && tmpSchemas.size() == 1
                    && (schemaFilters == null || schemaFilters.isNotApplicable())) {
    ...
    public GenericCatalog createCatalogImpl(
            @NotNull GenericDataSource dataSource, @NotNull String catalogName) {
     ...
     * </OL>
     */
    public JDBCStatement prepareLoadStatement(
            @NotNull JDBCSession session,
            @NotNull TurboGraphPPStructContainer owner,
            @Nullable TurboGraphPPTableBase object,
            @Nullable String objectName)
            throws SQLException {
        return session.getMetaData().getTables(null, null, "%", null).getSourceStatement();
    }
    ...
    public TurboGraphPPTableBase createTableImpl(
        ...
        if (CommonUtils.isEmpty(tableName)) {
            log.debug(
                    "Empty table name "
                            + (owner == null ? "" : " in container " + owner.getName()));
            return null;
        }
        ...
        TurboGraphPPTableBase table = this.createTableImpl(owner, tableName, tableType, dbResult);
        if (table == null) {
            return null;
        }
    ...
    public TurboGraphPPTableBase createTableImpl(
        ...
        if (tableType != null && tableType == "VIEW") {
    ...
    public String getViewDDL(
            DBRProgressMonitor monitor, TurboGraphPPView sourceObject, Map<String, Object> options)
            throws DBException {
        return "-- View definition not available";
    }

    public String getTableDDL(
            DBRProgressMonitor monitor, TurboGraphPPTable sourceObject, Map<String, Object> options)
            throws DBException {
    ...
    public TurboGraphPPTableColumn createTableColumnImpl(
            ...
            throws DBException {
        return new TurboGraphPPTableColumn(
                table,
                columnName,
                typeName,
                valueType,
